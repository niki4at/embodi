...
- Work on the `main` branch only; when committing, stage just the commit-worthy changes and gitignore or remove the rest.
...
- Convex OpenAI usage reads `OPENAI_MODEL` (currently GPT-5.6 Terra, with `reasoning.effort: 'none'` and `text.verbosity: 'low'` for fastest/cheapest replies) and `OPEN_API_KEY` from the Convex environment via `convex/openai.ts`; update the backend with `npx convex dev` or a deploy—Expo reload alone does not push Convex function changes. Set `OPENAI_MODEL` on the day-to-day Convex deployment (dev `helpful-gopher-816`) to the Terra model id.
...
- **Pre-launch hosting (intentional):** day-to-day data lives on Convex **dev** `helpful-gopher-816` (`https://helpful-gopher-816.convex.cloud` / `.convex.site`). A separate Convex **prod** deployment already exists and was used for an earlier site ship: `valiant-salamander-348` (`https://valiant-salamander-348.convex.cloud`). Until true go-live, `embodi.expo.app` is pointed at **dev** so Google sign-in shows the same history as the emulator/local app (prod was nearly empty and looked like a new account). When going live for real: (1) migrate or copy needed user data into `valiant-salamander-348` if required, (2) keep shipping backend with `npx convex deploy` to that prod deployment, (3) re-export the web app with `EXPO_PUBLIC_CONVEX_URL` / `EXPO_PUBLIC_CONVEX_SITE_URL` set to the prod URLs (clear Metro cache; verify the bundle embeds `valiant-salamander-348`, not `helpful-gopher-816`), then `npx eas-cli@latest deploy --prod`. Do not assume website = Convex prod until that cutover.
...
- **Website `embodi.expo.app`**: `npx expo export -p web` then `npx eas-cli@latest deploy --prod` (drop `--prod` for a throwaway preview URL). **Pre-launch exception:** the live site currently uses day-to-day Convex **dev** (`helpful-gopher-816`); Convex **prod** (`valiant-salamander-348`) already exists for the real go-live cutover (see Learned Workspace Facts).
...
All EAS commands need `EXPO_TOKEN` (or `eas login`) to run non-interactively; it's provided as a Cloud Agent secret (robot user `cursor-cloud-agent` on the `nick4eto` account — verify with `npx eas-cli@latest whoami`). After go-live, prod builds should read `EXPO_PUBLIC_CONVEX_URL` for Convex **prod** (`valiant-salamander-348`), not from `.env.local`/tunnels pointing at day-to-day data. Do the merge-to-`main` + true prod cutover only when the user explicitly says to ship / go live.
